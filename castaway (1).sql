...
--
-- Host: 127.0.0.1
-- Tempo de geração: 04/11/2025 às 23:08
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `BuscarvendedorPorID` (IN `vendedor_id` INT)   BEGIN
    SELECT * FROM vendedor WHERE id = vendedor_id;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `ListarArmaduras` ()   BEGIN
SELECT * FROM item WHERE item.FK_Id_tipo_item=tipo_item.PK_Id_tipo_item AND item.PK_Id_item=3;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `ListarArmas` ()   BEGIN
SELECT * FROM item WHERE item.FK_Id_tipo_item=tipo_item.PK_Id_tipo_item AND item.PK_Id_item=2;
END$$
...
SELECT * FROM item WHERE item.FK_Id_tipo_item=tipo_item.PK_Id_tipo_item AND item.PK_Id_item=1;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `ListarItensLucrativos` ()   BEGIN
    SELECT * FROM item WHERE item.PrecoV_item_item > item.PrecoC_item;
END$$
...
SELECT HP_inimigo, ATQ_inimigo, DEF_inimigo
FROM inimigo
WHERE PK_Id_inimigo = p_Id_inimigo;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `ListarStatusPlayer` (IN `pkey_Id_player` INT)   BEGIN
SELECT Nome_player, HP_player, ATQ_player, DEF_player, Level_player, Gold_player
FROM player
